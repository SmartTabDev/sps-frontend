...
export function useHotjar(): TUseHotjar {
  ...
  const initHotjar = React.useCallback(
    (
      ...
      if (logCallback && typeof logCallback === 'function') logCallback(`Hotjar ready: ${isHotjarAppended}`);
  ...
  const identifyHotjar = React.useCallback(
    (
      ...
      try {
        const formattedUserInfo = typeof userInfo !== 'string' ? JSON.stringify(userInfo) : userInfo;
        ...
        if (logCallback && typeof logCallback === 'function') logCallback(`Hotjar identified: ${isIdentified}`);
